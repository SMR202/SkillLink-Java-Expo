    ...
    return (
      <View style={s.container}>
        ...
        <View style={s.successWrap}>
          ...
          <GradientButton title="Back to Bookings" onPress={() => navigation.goBack()} style={{ marginTop: spacing.xl, width: '100%' }} />
        </View>
      </View>
    );
  }
    ...
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.content}>
        <View style={s.providerCard}>
          <View style={s.avatar}><Text style={s.avatarText}>{providerName?.[0]?.toUpperCase() || '?'}</Text></View>
        ...
        <View style={s.stars}>
          {[1, 2, 3, 4, 5].map(i => (
            <Pressable key={i} onPress={() => setRating(i)} style={s.starBtn}>
        ...
        <Text style={[s.label, { marginTop: spacing.xl }]}>Your Review</Text>
        <TextInput
          style={s.input}
        ...
        />
        <Text style={[s.counter, comment.length < 20 && { color: colors.error }]}>{comment.length}/1000 (min 20)</Text>
...
const s = StyleSheet.create({
  ...
  providerCard: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, backgroundColor: colors.bgCard, padding: spacing.lg, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.xl, ...shadows.sm },
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: colors.accent, justifyContent: 'center', alignItems: 'center' },
  avatarText: { ...typography.bodyLarge, color: '#FFF', fontWeight: '700' },
  providerName: { ...typography.bodyMedium, color: colors.textPrimary, fontWeight: '600' },
  label: { ...typography.bodyMedium, color: colors.textPrimary, fontWeight: '600', marginBottom: spacing.sm },
  stars: { flexDirection: 'row', gap: spacing.md, justifyContent: 'center', paddingVertical: spacing.md },
  starBtn: { padding: spacing.xs },
  star: { fontSize: 40, color: colors.border },
  starActive: { color: colors.star },
  ratingLabel: { ...typography.small, color: colors.star, textAlign: 'center', fontWeight: '600' },
  input: { backgroundColor: colors.bgInput, borderRadius: borderRadius.md, padding: spacing.lg, minHeight: 120, ...typography.body, color: colors.textPrimary, borderWidth: 1, borderColor: colors.border },
  counter: { ...typography.caption, color: colors.textMuted, textAlign: 'right', marginTop: spacing.xs },
  successWrap: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: spacing.xxl },
  successTitle: { ...typography.h2, color: colors.textPrimary, marginTop: spacing.lg },
